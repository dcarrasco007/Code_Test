...
$mysqli ->  set_charset("utf8");
    ...
    $puertos=explode("|",$puerta);
    print_r($puertos);
    $sql_ip = "SELECT OLT_SERVER.ip, OLT_SERVER.region, OLT_SERVER.modelo, OLT_SERVER.comuna FROM OLT_SERVER WHERE OLT_SERVER.`server`='$server'";
    $result = $mysqli->query($sql_ip) or die("error $sql_ip");
    $rowInicial = $result->fetch_array(MYSQLI_NUM);
    $ip = $rowInicial[0];
    $modelo = $rowInicial[2];
    $region = $rowInicial[1];
    ...
    $cont_poder = 0;
    $cont_con = 0;
//-------ELIMINA REGISTROS DE PUERTA CONMPLETA

for ($e=0; $e <count($puertos) ; $e++) {
    if($puertos[$e]!=''){
        $divisionesE=explode("/",$puertos[$e]);
        echo $sql_borrado = "DELETE FROM OLT_INFORMACION_ONT_DETALLE_COMPLETO WHERE equipo ='$server' AND frame_id='$divisionesE[0]' AND slot_id='$divisionesE[1]' AND port_id='$divisionesE[2]'";
        $result = $mysqli->query($sql_borrado) or die("error $sql_borrado");
    }
}
    ...
    function obtenerData2($log,$server){
        ...
        foreach ($ont_data as $port => $data) {
            ...
            while ($row = $result1->fetch_array(MYSQLI_NUM)) {
                ...
                echo $query="INSERT INTO Aden.OLT_INFORMACION_ONT_DETALLE_COMPLETO
                ...
                if ($result) {
                    echo "\nInserción : ".$port." ID ONT: ".$row[4]." || ";
                } else {
                    echo"\n------------ Error en la consulta: " . $mysqli->error . " - Query: " . $query;
                }
