...
function estado_equipo($server,$modelo){
    ...
    while ($x){
        ...
        case SHELL_CONFIG:
            if ($i==0) {
                fwrite($stream, "display alarm active alarmlevel critical\n");
                sleep(1);
            }
            if ($i==1) {
                if($modelo=='MA5800-X15'){
                    fwrite($stream, "display alarm history alarmlevel cleared alarmclass recovery detail\n");
                }else{
                    fwrite($stream, "display alarm history alarmclass recovery alarmlevel critical detail\n");
                }
                sleep(1);
            }
            if ($i>=2) {
