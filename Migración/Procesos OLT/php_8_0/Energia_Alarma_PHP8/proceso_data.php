    ...
    while ($x)
    {
            ...
            ), $match))
        {
            ...
            case SHELL_CONFIG:
                if ($i==0) {
                    sleep(1);
                    expect_send($stream, "display alarm history alarmlevel minor detail\n");
                }
                if ($i==1) {
                    sleep(3);
                    expect_send($stream, "display alarm history alarmlevel cleared detail\n");
                }
                if($i>=2){
                    sleep(1);
                    expect_send($stream, "quit\n");
    ...
    while ($x)
    {
            ...
            ), $match))
        {
            ...
            case SHELL_CONFIG:
                if ($i==0) {
                    sleep(1);
                    expect_send($stream, "display alarm active alarmlevel major detail\n");
                }
                if ($i==1) {
                    sleep(3);
                    expect_send($stream, "display alarm history alarmlevel major detail\n");
                }
                if($i>=2){
                    sleep(1);
                    expect_send($stream, "quit\n");
