...
$sql_ip = "SELECT
    OLT_POTENCIA_OPTICA_UPLINK.`equipo`,
    OLT_POTENCIA_OPTICA_UPLINK.ip,
    OLT_POTENCIA_OPTICA_UPLINK.modelo,
    OLT_POTENCIA_OPTICA_UPLINK.region
    FROM 
    OLT_POTENCIA_OPTICA_UPLINK
    WHERE OLT_POTENCIA_OPTICA_UPLINK.puerta ='0/0/0' AND OLT_POTENCIA_OPTICA_UPLINK.potencia_tx =0 AND OLT_POTENCIA_OPTICA_UPLINK.potencia_rx =0";
$result = $mysqli->query($sql_ip) or die("error 1");

$sql_elimina="DELETE FROM OLT_POTENCIA_OPTICA_UPLINK WHERE OLT_POTENCIA_OPTICA_UPLINK.puerta ='0/0/0' AND OLT_POTENCIA_OPTICA_UPLINK.potencia_tx =0 AND OLT_POTENCIA_OPTICA_UPLINK.potencia_rx =0";
$resultado_eliminados = $mysqli->query($sql_elimina) or die("error elimina");
$ERROR=0;
$ERROR2=0;
$semana=date('W');
            ...
            case SHELL_CONFIG1:
                        ...
                        if($puerto07>0){
                            for ($x=0; $x<$puerto07; $x++) { 
                                ...
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                        ...
                        if($puerto19>0){
                            for ($x=0; $x<$puerto19; $x++) { 
                                expect_send($stream, "interface giu 0/19\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                            ...
                            for ($x=0; $x<$puerto20; $x++) { 
                                expect_send($stream, "interface giu 0/20\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
    ...
    while ($x)
    {
            ...
            ), $match))
        {
            ...
            case SHELL_CONFIG1:
                        if($puerto16>0){
                            for ($x=0; $x<$puerto17; $x++) { 
                                expect_send($stream, "interface eth 0/16\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                        ...
                        if($puerto19>0){
                            for ($x=0; $x<$puerto19; $x++) { 
                                expect_send($stream, "interface eth 0/19\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                            ...
                            for ($x=0; $x<$puerto20; $x++) { 
                                expect_send($stream, "interface eth 0/20\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                        ...
                        if($puerto18>0){
                            for ($x=0; $x<$puerto18; $x++) { 
                                expect_send($stream, "interface eth 0/18\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                        ...
                        if($puerto17>0){
                            for ($x=0; $x<$puerto17; $x++) { 
                                expect_send($stream, "interface eth 0/17\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                            ...
                            for ($x=0; $x<$puerto08; $x++) { 
                                expect_send($stream, "interface mpu 0/8\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
                            ...
                            for ($x=0; $x<$puerto09; $x++) { 
                                expect_send($stream, "interface mpu 0/9\n");
                                expect_send($stream, "display port ddm-info $x\n");
                                $uname .= $match[0];
                                expect_send($stream, "\n");
                                expect_send($stream, "quit\n");
