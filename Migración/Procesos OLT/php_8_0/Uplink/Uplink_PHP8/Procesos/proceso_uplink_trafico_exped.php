...
// --- Migración a PHP 8.0 -------------------------------------------------
// En PHP 5.3 una constante no definida se evaluaba como su propio nombre en
// texto (con un aviso). En PHP 8.0 eso es un error FATAL. Estas constantes se
// usan como etiquetas de los switch de expect_expectl; se definen aquí con su
// nombre como valor para reproducir EXACTAMENTE el comportamiento original.
foreach (['USER','PASSWORD','SALTO','SHELL','SHELL_CONFIG','SHELL_CONFIG1','SHELL2','SALIR','ESPACIO','ESPACIO2'] as $__const) {
...
//$hora = date ( 'Y-m-d');
    ...
    $contador08=0;
    $contador09=0;
    $ERROR=0;  // Migración PHP 8.0: inicializada (antes se usaba sin definir -> Warning)
    $ERROR2=0; // Migración PHP 8.0: inicializada (antes se usaba sin definir -> Warning)
if(isset($argv[8])){
...
echo "Fin telnet: ".date("Y-m-d H:i:s")."\n";
                ...
                foreach (explode(chr(13), $texto) as $linea){
                    $linea = preg_replace('/[\n|\r|\n\r]/', '', $linea); // Migración PHP 8.0: eregi_replace() fue eliminada en PHP 7
                    $data[] = $linea;
                }
                $trafico16=0;
                $trafico17=0;
...
function estado_equipo2($server,$puerto){
    ...
    while ($x)
    {
            ...
            ), $match))
        {
            ...
            case SHELL_CONFIG:
                if ($i==0) {
                    expect_send($stream,"display port vlan $puerto\n");
                }
                if ($i==1) {
                    expect_send($stream,"display port vlan $puerto\n");
                }
