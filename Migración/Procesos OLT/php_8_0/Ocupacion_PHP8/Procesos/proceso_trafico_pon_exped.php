            ...
            case SHELL_CONFIG1:
                if($t<count($puertas)){
                    //echo "entro puerta: ".$puertas[$t];
                        sleep(2);
                        expect_send($stream, "interface gpon 0/$puertas[$t]\n");
                        sleep(2);
                        for ($i=0; $i < 16; $i++) { 
                            echo "puerta: 0/".$puertas[$t]."/".$i."\n";
                            sleep(1);
                            expect_send($stream, "display port traffic $i\n");
                            sleep(1);
                            expect_send($stream, "\n");
                         ...
                         /* if($t==0){
                           ...
                           for ($p=0; $p <count($puertas) ; $p++) { 
                            echo "entro puerta: ".$puertas[$p];
                                sleep(1);
                                expect_send($stream, "interface gpon 0/$puertas[$p]\n");
                                sleep(1);
                                for ($i=0; $i < 16; $i++) { 
