...
mysql_query('TRUNCATE TABLE OLT_UPLINKS_STATE');
        ...
        while($data[$key+$k2] != '  ------------------------------------------------------------------------------'){
            ...
            if($parametros[0] == 0){
                if($inter != 'Failure:'){
                    ...
                    $resultado = mysql_query($sql) or die ("error shell_uplinks_state.php 1 $sql".mysql_error());
                }
                ++$k2;
                $k = 0;
    ...
    while ($x)
    {
            ...
            ), $match))
        {
            ...
            case SHELL_CONFIG: //display port ddm-info 0
                if ($i==0) {
                    fwrite($stream, "$comando1 $puerto\n");
                }
                if ($i==1) {
                    fwrite($stream, "$comando2\n $comando3\n $comando4\n quit\n");
                }
                if ($i==2) {
                    fwrite($stream, "\n");
                }
