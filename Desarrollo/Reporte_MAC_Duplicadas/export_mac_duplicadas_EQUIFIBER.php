<?php
// Exporta el reporte de SN/MAC duplicadas (solo OLTs EQUIFIBER) a .xlsx (2 hojas),
// replicando el formato/colores del informe de referencia.
...
$sql = "SELECT sn_mac, COUNT(*) AS veces,
        ...
        INNER JOIN OLT_SERVER_V2 ON OLT_SERVER_V2.server = OLT_INFORMACION_ONT_DETALLE_COMPLETO.equipo
        GROUP BY sn_mac
        HAVING COUNT(*) > 1
        ORDER BY sn_mac ASC";
...
$sheet->setTitle('MACs Duplicadas EQUIFIBER');
...
// Titulo
$sheet->setCellValue('A1', 'ONUs con SN/MAC en más de un puerto (EQUIFIBER) — Access ' . $fechaHoy);
...
$sheet2->setCellValue('A1', 'Resumen por Estado Running (EQUIFIBER)');
...
// ================== Descarga ==================
$nombreArchivo = 'MACs_duplicadas_EQUIFIBER_' . date('Ymd_His') . '.xlsx';
